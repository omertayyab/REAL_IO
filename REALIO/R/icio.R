...
icio <- vroom("ICIO/2020_SML.csv")
conc <- vroom("ICIO/ICIO_BEC_conc.csv", n_max = 44) %>% select(1:4)
...
cntry_list <- read_csv("ICIO/ICIO_BEC_conc.csv") %>% select(7:9)
conc <- read_csv("ICIO/ICIO_BEC_conc.csv", n_max = 44) %>% select(1:4)
conc$Name <- paste(conc$Name, conc$BECv5)
...
p5 <- sankeyNetwork(Links = link_sel ,
                    Nodes = nodes_sel,
                    Source = "source",
                    Target = "target", Value = "value", NodeID = "names",
                    LinkGroup = "group", colourScale=my_color,
